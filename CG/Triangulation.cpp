...
// 其中使用了大量的近似值 我认为是在浮点计算中 可能产生误差 所以给了容错率 EPS

struct Point {// 二维的三角剖分
  double x, y;
  int id;

  Point(double a = 0, double b = 0, int c = -1) : x(a), y(b), id(c) {}

  bool operator<(const Point &a) const {// 保证点是从左至右 从下至上 进行排序
  ...
  double dist2(const Point &b) {// 计算两点之间的欧氏距离
    return (x - b.x) * (x - b.x) + (y - b.y) * (y - b.y);
  }
};

struct Point3D {// 三维的三角剖分
  double x, y, z;
...
int cmp(double v) { return fabs(v) > EPS ? (v > 0 ? 1 : -1) : 0; }// 暂时放着

double cross(const Point &o, const Point &a, const Point &b) {// ？
  return (a.x - o.x) * (b.y - o.y) - (a.y - o.y) * (b.x - o.x);
}

Point3D cross(const Point3D &a, const Point3D &b) {// 三维叉乘
  return Point3D(a.y * b.z - a.z * b.y, -a.x * b.z + a.z * b.x,
                 a.x * b.y - a.y * b.x);
}

int inCircle(const Point &a, Point b, Point c, const Point &p) {// 判断 
...
class Delaunay {
 public:
  ...
  void init(int n, Point p[]) {
    memcpy(this->p, p, sizeof(Point) * n);// 将数组 p[] 的值赋值给 Delaunay 对象内的 p 数组
    std::sort(this->p, this->p + n);// 将点按照从左到右 从下到上 进行排序
    for (int i = 0; i < n; i++) rename[p[i].id] = i;// 将排好序的数组对 id 进行赋值
    this->n = n;
    divide(0, n - 1);
  }
  ...
  void divide(int l, int r) {// 递归的将区间划分连边 区间内只剩 3 个点以内
    if (r - l <= 2) {  // #point <= 3
      for (int i = l; i <= r; i++)
